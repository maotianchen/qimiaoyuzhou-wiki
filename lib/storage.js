...
function writePage(title, content, { originalTitle, author, birthday } = {}) {
  const name = safeFileName(title);
  ...
  const data = {
    ...
    author: (author !== undefined ? author : (existing ? existing.author : '')) || '',
    birthday: (birthday !== undefined ? birthday : (existing ? existing.birthday : '')) || '',
  };
  ...
  if (data.birthday) meta.push(`birthday: ${data.birthday}`);
  const body = `---\n${meta.join('\n')}\n---\n\n${content}`;
